# -*- coding: utf-8 -*-
# @Time    : 2024/8/7 下午10:48
# @Author  : Yancan Chen
# @Email   : yancan@u.nus.edu
# @File    : all_process_clip_prompt_only.py.py
...
def question_attribute_navigation_llava(query, model=None, tokenizer=None):
    """
    Generate text using a pretrained model.

    Args:
        prompt (str): Input text to start the generation.
        model_name (str): Model identifier on Hugging Face Model Hub.
...
def chat_search_step1(recall_res, output_file_path_step1,
    ...
    for i in tqdm(range(start_index, len(recall_res))):
        ...
        try:
            ...
            # 提问题
            question, common_entities, diff_entities = question_attribute_navigation_llava(query,
                                                                                           model=llava_model,
...
def load_llave_model_processor(model_name):
...
if __name__ == "__main__":
    ...
    tokenizer, model = load_llave_model_processor(model_name)

    # Modify the model path
    dataset_name = 'visdial'  # 'mscoco'
    ...
    output_file_path_step1 = f'./rank_res/step1_clip_vicuna-7b_{dataset_name}_prompt_only.jsonl'
    output_file_path_step2 = f'./rank_res/clip_vicuna-7b_{dataset_name}_prompt_only.jsonl'
    ...
    llava_tokenizer, llava_model = load_llave_model_processor(llava_model_id)
    ...
    if run_autodl:
        chat_search_step1(recall_res, output_file_path_step1, image_path_base_dir,
